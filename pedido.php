...
if (isset($_GET['email'])) {
    ...
    $getidClienteEmail = "select * from clientes where email = '$email'";
    $run_cliente_email = mysqli_query($con, $getidClienteEmail);
    $results = mysqli_fetch_array($run_cliente_email);
    $cliente = $results['id'];
    ...
    $sql = "update clientes set session_id ='$session_id', dtmodification = NOW()  where id = '$cliente'";
    ...
    $runCart = mysqli_query($con, $select_cart);
    while ($row = mysqli_fetch_array($runCart)) {
        $pro_id = $row['id_pro'];
        $pro_size = $row['p_size'];
        $qtd = $row['quantidade'];
